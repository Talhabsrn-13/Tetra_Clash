...
public class GamePlay : MonoBehaviour, IPointerDownHandler, IPointerUpHandler,IBeginDragHandler,IDragHandler
{
	...
	public ShapeInfo currentShape = null;
    ...
    void Start ()
	{
		...
		player.GameBoardGenerator.GenerateBoard ();
	...
	public void OnPointerDown (PointerEventData eventData)
	{
		if (eventData.pointerCurrentRaycast.gameObject != null)
		{
			...
			if (clickedObject.GetComponent<ShapeInfo>() != null && !isTouched) 
			{
				isTouched = true;
	...
	{
		if (currentShape != null && isTouched)
		{
			isTouched = false;
			...
			else
			{
				...
				currentShape.transform.localScale = new Vector3 (0.6f,0.6f,0.6f);
				currentShape = null;
    ...
    public void OnPointerDown(Vector2 touchPosition)
	{
        ...
        if (hit.collider != null && hit.collider.GetComponent<ShapeInfo>() != null && !isTouched)
        {
			isTouched = true;
			currentShape = hit.collider.gameObject.GetComponent<ShapeInfo>();
			currentShape.transform.localScale = Vector3.one * player.GameBoardGenerator.BlockSize;
			currentShape.transform.localPosition = new Vector3(touchPosition.x, touchPosition.y, 0);
			AudioManager.Instance.PlaySound(blockSelectSound);
		}
    
	}

	public void OnBeginDrag(Vector2 touchPosition)
	{
		if (currentShape != null)
		{
			currentShape.transform.localPosition = touchPosition;
		}
	}
	...
	public void OnDrag(Vector2 touchPosition)
	{
		if (currentShape != null)
		{
			...
			currentShape.transform.position = new Vector3(touchPosition.x, touchPosition.y, 0F);
    ...
    public bool CanPlaceShape (Transform currentHittingBlock)
	{
		...
		{
			Block checkingCell = blockGrid.Find (o => o.rowID == currentRowID + (c.rowID + currentShape.startOffsetX) && o.columnID == currentColumnID + (c.columnID - currentShape.startOffsetY)&& currentCell.PlayerType == c.PlayerType);
	...
	void CheckBoardStatus()
	{
		...
		{
			player.BlockShapeSpawner.FillShapeContainer ();
			player.ScoreManager.AddScore (10 * placingShapeBlockCount);
		}
	...
	List<Block> GetEntireRow(int rowID)
	{
		...
		for (int columnIndex = 0; columnIndex < player.GameBoardGenerator.TotalColumns; columnIndex++) {
	...
	List<Block> GetEntireColumn(int columnID)
	{
		...
		for (int rowIndex = 0; rowIndex < player.GameBoardGenerator.TotalRows; rowIndex++) {
	...
	IEnumerator BreakAllCompletedLines(List<List<Block>> breakingRows, List<List<Block>> breakingColumns, int placingShapeBlockCount)
	{
		...
		if (TotalBreakingLines == 1)
		{
			...
			player.ScoreManager.AddScore (100 + (placingShapeBlockCount * 10));
		}
		else if (TotalBreakingLines == 2)
		{
			AudioManager.Instance.PlaySound (lineClear2);
			player.ScoreManager.AddScore (300+ (placingShapeBlockCount * 10));
		}
		else if (TotalBreakingLines == 3)
		{
			AudioManager.Instance.PlaySound (lineClear3);
			player.ScoreManager.AddScore (600+ (placingShapeBlockCount * 10));
		}
		else if (TotalBreakingLines >= 4)
		{
			...
			{
				player.ScoreManager.AddScore (1000+ (placingShapeBlockCount * 10));
			}
			else
			{
				player.ScoreManager.AddScore ((300 * TotalBreakingLines)+ (placingShapeBlockCount * 10));
			}
		}
	...
	{
		//todo Game over screen active;
		//todo Multiplayer Manager Event activate;
		GetComponentInParent<MultiplayerManager>().OnPlayerLose(this.player);
        //if ((TotalRescueDone < MaxAllowedRescuePerGame) || MaxAllowedRescuePerGame < 0)
        //{
        //    //player.GamePlayUI.ShowRescue (GameOverReason.OUT_OF_MOVES);
        //}
        //else
        //{
	...
	public void OnBombCounterOver(){
		if ((TotalRescueDone < MaxAllowedRescuePerGame) || MaxAllowedRescuePerGame < 0)
		{
			player.GamePlayUI.ShowRescue (GameOverReason.BOMB_COUNTER_ZERO);
		} else {
			Debug.Log("GameOver Called..");
			OnGameOver ();
		}
	...
	void ExecuteRescue()
	{
		//if (player.GamePlayUI.currentGameOverReson == GameOverReason.OUT_OF_MOVES) {
		//	int TotalBreakingColumns = 3;
		//	int TotalBreakingRows = 3;

		//	int totalColumns = player.GameBoardGenerator.TotalColumns;
		//	int totalRows = player.GameBoardGenerator.TotalRows;
		...
		//#region bomb mode
		//if ((GameController.gameMode == GameMode.BLAST || GameController.gameMode == GameMode.CHALLENGE) && player.GamePlayUI.currentGameOverReson == GameOverReason.BOMB_COUNTER_ZERO) {
		...
		//if (GameController.gameMode == GameMode.TIMED || GameController.gameMode == GameMode.CHALLENGE) 
		//{
		//	if(player.GamePlayUI.currentGameOverReson == GameOverReason.TIME_OVER)
		//	{
	...
	public void OnGameOver()
	{
		...
		gameOverScreen.GetComponent<GameOver> ().SetLevelScore (player.ScoreManager.Score, 10);
		player.GameProgressManager.ClearProgress();
	...
	void PlaceAtRandomPlace()
	{
		...
		if (BombPlacingRows.Count > 0) {
			...
			RandomRowForPlacingBomb = UnityEngine.Random.Range (0, player.GameBoardGenerator.TotalRows);
		}
