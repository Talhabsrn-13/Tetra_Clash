...
public class BlockShapeSpawner : MonoBehaviour
{
	...
	{
		if (player.GameBoardGenerator.previousSessionData != null) {
			List<int> shapes = player.GameBoardGenerator.previousSessionData.shapeInfo.Split(',').Select(Int32.Parse).ToList();
	...
	public void AddRandomShapeToContainer(Transform shapeContainer)
	{
		...
		spawningShapeBlock.transform.localRotation = Quaternion.identity;
		spawningShapeBlock.GetComponent<RectTransform>().anchoredPosition3D = Vector3.zero;
		#if HBDOTween
	...
	public void CheckOnBoardShapeStatus()
	{
		...
		bool canExistingBlocksPlaced = player.GamePlay.CanExistingBlocksPlaced (OnBoardBlockShapes);

		if (canExistingBlocksPlaced == false)
		{
			player.GamePlay.OnUnableToPlaceShape ();
		}
	}
