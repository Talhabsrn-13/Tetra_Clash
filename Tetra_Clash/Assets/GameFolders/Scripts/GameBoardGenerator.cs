...
public class GameBoardGenerator : MonoBehaviour
{
	...
	{	
		...
		for (int row = 0; row < TotalRows; row++) {
			...
			player.GamePlay.blockGrid.AddRange (thisRowCells);
			newPosX = startPosx;
	...
	void SetupPreviousSessionBoard()
	{
		if (previousSessionData != null) 
		{
			...
			if (previousSessionData.blockGridInfo.Count > 0) 
			{
				...
				foreach (string gridRow in previousSessionData.blockGridInfo)
				{
					...
					foreach (string blockID in gridRow.Split(','))
					{
						...
						if (thisBlockID >= 0)
						{
							Block thisBlock = player.GamePlay.blockGrid.Find (o => o.rowID == rowIndex && o.columnID == columnIndex);
							if (thisBlock != null)
							{
			...
			if(GameController.gameMode == GameMode.BLAST || GameController.gameMode == GameMode.CHALLENGE)
			{
				if (previousSessionData.placedBombInfo != null) {
					foreach(PlacedBomb bomb in previousSessionData.placedBombInfo) {
						Block thisBlock = player.GamePlay.blockGrid.Find (o => o.rowID == bomb.rowID && o.columnID == bomb.columnID);
			...
			#region set score
			player.ScoreManager.AddScore(previousSessionData.score,false);
			#endregion

			#region moves count
			player.GamePlay.MoveCount = previousSessionData.movesCount;
			#endregion
			...
			{
				player.GamePlay.timeSlider.SetTime(previousSessionData.remainingTime);
			}
			#endregion
		}
	...
	GameObject GenerateNewBlock (int rowIndex, int columnIndex)
	{
		...
		newBlock.GetComponent<Block>().PlayerType = player.PlayerType;
